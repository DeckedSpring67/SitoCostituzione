	...
	<body>
		...
		function hemicycle(options){
			h = {
				...
				height:550,
				width:950,
				partyorder:{'comunista':1,'dc':2,'ddl':3,'repubblicano':4,'liberale':5,'autonomisti':6,'uq':7,'ps':8,'uz':9,'udn':10,'altri':11},
				datasize:556,
		...
		function info(x)
		{
			...
			if(completo)
			{
				partitoText.setAttribute("transform", "translate(" +dizionario[x.getAttribute("name")].length*5.5  + ",50) rotate(180)");
				svg.appendChild(partitoText);
				seggiText.setAttribute("transform", "translate(" +dizionario[x.getAttribute("name")].length*5.5  + ",20) rotate(180)");
		...
		function premuto(x){
			...
			if(x){//I 75
				...
				for(i=0;i < pallini.length; i++){
					...
					{
						case "comunista" : comunisti.push(pallini[i]); break;
						case "dc" : dc.push(pallini[i]); break;
						case "ddl" : ddl.push(pallini[i]); break;
						case "repubblicano" : repubblicani.push(pallini[i]); break;
						case "liberale" : liberali.push(pallini[i]); break;
						case "autonomisti" : autonomisti.push(pallini[i]); break;
						case "uq" : uq.push(pallini[i]); break;
						case "ps" : ps.push(pallini[i]); break;
						case "uz" : uz.push(pallini[i]); break;
						case "udn" : udn.push(pallini[i]); break;
						case "altri" : altri.push(pallini[i]); break;
					}
				}
				comunisti.shuffle();
				dc.shuffle();
