...
<body>
	...
	<div class="container" style="margin-top:11em;margin-bottom:5em;">
		...
		<div class="container" style="text-align:center;" id="emiciclo">
			...
			console.log(distanzax+"   x");
			console.log(distanzay+"   y");
			console.log(posxstart+"   p");
			console.log(posxscritte+"   xs");
			console.log(posyscritte+"   ys");
			console.log(fontbig);
			...
			//
				...
				function info(x) {
					...
					if (completo) {
						...
						partitoText.setAttribute("transform", "translate(" + dizionario[x.getAttribute("name")].length *((screen.width*0.3)/100) +
							","+((screen.height*4.629)/100)+") rotate(180)");
						svg.appendChild(partitoText);
						seggiText.setAttribute("transform", "translate(" + dizionario[x.getAttribute("name")].length *((screen.width*0.3)/100) +
							","+((screen.height*1.851)/100)+") rotate(180)");
						svg.appendChild(seggiText);
					} else {
						var parlamentari_partito = document.getElementsByName(x.getAttribute("name"));
						var attivi = [];
						for (i = 0; i < parlamentari_partito.length; i++) {
							if (!parlamentari_partito[i].getAttribute("style").includes("visibility: hidden;")) {
								attivi.push(parlamentari_partito[i]);
							}
						}
						seggiText.textContent = seggi[x.getAttribute("name")][0].split(" ")[0] + " " + attivi.length;
						partitoText.setAttribute("transform", "translate("+((screen.width*21.093)/100)+","+((screen.height*18.981)/100)+") rotate(180)");
						svg.appendChild(partitoText);
						seggiText.setAttribute("transform", "translate("+((screen.width*21.093)/100)+","+((screen.height*17.129)/100)+") rotate(180)");
						svg.appendChild(seggiText);
					}
				...
				function premuto(x) {
					...
					if (x) { //I 75
						...
						cs3.setAttribute("transform", "translate("+posyscritte+","+(posxscritte-distanzax-distanzax)+") rotate(180)");
						svg.appendChild(cs3);
				...
				function sposta(n1, n2, n3) {
					...
					while (cont < fff + n2) {
						k = document.getElementById(i75[cont].seatname);
						k.style.transform = "translate(0,0)";
						k.style = "r: \"dotsize\"; transition: all 0.8s; transform: translate(" + r2 + "px,"+(posxstart-distanzax)+"px);";
						r2 = r2 - distanzay;
						cont++;
					}
					fff = cont;
					while (cont < fff + n3) {
						k = document.getElementById(i75[cont].seatname);
						k.style.transform = "translate(0,0)";
						k.style = "r: \"dotsize\"; transition: all 0.8s; transform: translate(" + r3 + "px,"+(posxstart-distanzax-distanzax)+"px);";
						r3 = r3 - distanzay;
						cont++;
					}
			...
			<center><h3 id="BrowserEdge"></h6></center> 
			<script>
				var dispositivo = navigator.userAgent;
				if(dispositivo.includes("Edge"))
				{
					var paragraph = document.getElementById("BrowserEdge");
					var text = document.createTextNode("Alcune animazioni potrebbero non funzionare correttamente, sei pregato di usare Google Chrome");
